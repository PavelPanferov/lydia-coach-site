...
export const SectionHeading: React.FC<SectionHeadingProps> = ({ title, subtitle, centered = false, light = false, accent = false }) => {
  return (
    <div className={`mb-16 ${centered ? 'text-center mx-auto' : ''} max-w-4xl`}>
      ...
      {subtitle && (
        <div className={`flex ${centered ? 'justify-center' : 'justify-start'}`}>
          <div className={`h-[2px] w-12 ${light ? 'bg-red-500' : 'bg-red-600'} mt-3 mr-4`}></div>
